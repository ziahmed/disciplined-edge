...
GET  /          -> preview page (now: type ANY NASDAQ ticker)
GET  /health    -> {"status": "ok"}
GET  /docs      -> interactive API docs
POST /predict   -> prediction contract
"""
...
<div class="wrap">
  ...
  <p class="sub">Type any NASDAQ ticker &mdash; trained live, then cached.</p>

  <div class="controls">
    <div>
      <label for="symbol">Ticker</label>
      <input id="symbol" value="ASML" maxlength="6" autocomplete="off" spellcheck="false" />
    </div>
    <div>
  ...
  <div id="loading" class="sub" style="display:none">Training model &mdash; first view of a ticker can take 20-60s&hellip;</div>

  <p class="hint">
    First view of a ticker trains a model (slow); after that it's cached and
    instant. The IC and odds are real &mdash; expanding the universe is itself the
    experiment: do new stocks cluster near zero too?
  </p>
</div>
...
async function getPrediction(){
  ...
  try {
    ...
    if (!res.ok) {
      ...
      const msg = (e.detail && e.detail.detail) ? e.detail.detail : ('Error ' + res.status);
      out.innerHTML = '<p class="err">' + msg + '</p>';
